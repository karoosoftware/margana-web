...
const props = defineProps({
  ...
  pbValues: { type: Array, required: true },  // personal best user_cumulative
  title: { type: String, default: 'You vs Personal Best' },
  youLabel: { type: String, default: 'You' },
  pbLabel: { type: String, default: 'Personal Best' },
  beginAtZero: { type: Boolean, default: true },
  grace: { type: String, default: '5%' },
  days: { type: Array, default: () => [] },
  today: { type: String, default: '' },
  pbWeekId: { type: String, default: '' },
})
...
const chartData = computed(() => ({
  ...
  datasets: [
    ...
    {
      label: props.pbLabel,
      data: (props.pbValues || []).map(toNumOrNaN),
...
const dataLabelsPlugin = {
  ...
  afterDatasetsDraw(chart, args, options) {
    ...
    // Draw PB Week ID at bottom right of the chart area
    if (props.pbWeekId) {
      const pbWeekLabel = `WEEK#${props.pbWeekId}`
      ctx.save()
      ctx.font = 'bold 13px sans-serif'
      ctx.fillStyle = 'rgba(199, 210, 254, 0.5)' // Subtle light indigo text
      ctx.textAlign = 'right'
      ctx.textBaseline = 'bottom'

      const chartArea = chart.chartArea
      // Position it at the bottom right of the chart area with some padding
      const x = chartArea.right - 10
      const y = chartArea.bottom - 10

      ctx.fillText(pbWeekLabel, x, y)
      ctx.restore()
    }
  }
}
